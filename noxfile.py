...
def tests(session):
    ...
    try:
        ...
        session.run(
            ...
            "--omit=src/saltext/dell/*",
            "--include=tests/*",
        )
        try:
            session.run("coverage", "report", "--show-missing", "--include=src/saltext/dell/*")
            ...
            #    "--include=src/saltext/dell/*,tests/*"
            # )
        finally:
...
def gen_api_docs(session):
    ...
    session.run(
        ...
        "docs/ref/",
        "src/saltext",
        "src/saltext/dell/config/schemas",
    )
